...
def IterativeDepthFirstSearch(graph,src,visited):
    st=[]
    st.append([src,str(src)+""])
    while(len(st)>0):
        rem=st.pop()
        ...
        visited[rem[0]]=True
        print(rem[0],"--",rem[1])
        edge=graph.get_node(rem[0])
        while(edge):
            if(visited[edge.nbr]==False):
                st.append([edge.nbr,rem[1]+str(edge.nbr)])
            edge=edge.next
    

IterativeDepthFirstSearch(graph_,0,bool_arr)   
